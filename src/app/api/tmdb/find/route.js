...
function gate() {
  ...
  if (isBackendDisabled()) {
    return getBackendDisabledResponse('TMDB find requires the backend');
  }
  return null;
}
...
export async function GET(request) {
  ...
  try {
    ...
    const url = new URL(`${BACKEND_URL}/api/tmdb/find`);
    url.searchParams.set('authId', authId);

    for (const key of ['imdbId', 'tmdbId', 'mediaType', 'allowTmdbFallback']) {
    ...
    console.error('Error proxying TMDB find:', error);
