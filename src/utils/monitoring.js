...
const logSecurityEvent = (event, details = {}) => {
  ...
  const safeDetails = {
    ...details,
    authIds:
      details.authIds && Array.isArray(details.authIds)
        ? details.authIds.map((id) =>
            id && typeof id === 'string' ? id.substring(0, 8) + '...' : id
          )
        : details.authIds,
    owners:
      details.owners && Array.isArray(details.owners)
        ? details.owners.map((id) =>
            id && typeof id === 'string' ? id.substring(0, 8) + '...' : id
          )
        : details.owners,
    apiKey:
      details.apiKey && typeof details.apiKey === 'string'
