...
Related (unchanged): `UPLOAD_PROCESSOR_INTERVAL_MS` (scheduler tick, default 5s), `UPLOAD_PROCESS_CONCURRENCY` (cross-user worker slots, default 6), `UPLOAD_CONNECTION_DEFER_MS` (defer when TorBox is unreachable, default 15 min). Hourly create limits come from TorBox `x-ratelimit-*` response headers (cached in-memory per user/type).
