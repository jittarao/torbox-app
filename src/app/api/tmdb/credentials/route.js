...
export async function GET() {
  ...
  try {
    ...
    const url = new URL(`${BACKEND_URL}/api/tmdb/credentials`);
    ...
    console.error('Error reading TMDB credentials:', error);
...
export async function PUT(request) {
  ...
  try {
    ...
    // Allow time for backend TMDB validation retries (Bun TLS to api.themoviedb.org can flake)
    const response = await fetch(`${BACKEND_URL}/api/tmdb/credentials`, {
      cache: 'no-store',
      method: 'PUT',
    ...
    console.error('Error saving TMDB credentials:', error);
    return NextResponse.json({ success: false, error: sanitizeError(error) }, { status: 500 });
  }
}
...
export async function DELETE() {
  ...
  try {
    ...
    console.error('Error deleting TMDB credentials:', error);
    return NextResponse.json({ success: false, error: sanitizeError(error) }, { status: 500 });
  }
}
